...
function newForm( form ) {
  ...
  function stepAt ( index ) { return steps[ index ] }
  ...
  function forward ( timer ) {
    ...
    if ( nextStep() ) {
      stepIndex = nextStep().dataset.index
...
Event.ready( function(){
  ...
  var jumpSelector = '.progressive-form-nav [data-step]'

  Event.on( document, 'click', nextSelector , fire )
  Event.on( document, 'click', backSelector , function( event ) { goTo( previousStep() ) } )
  Event.on( document, 'click', jumpSelector , function( event ) { goTo( stepAt( event.currentTarget.dataset.step ) ) } )
