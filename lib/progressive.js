...
function newForm( form ) {
  ...
  function goTo ( index ) {
    ...
    if ( step && step != currentStep() ) {
      ...
      if ( active() ) {
        return dismiss( function() { goTo( index ) }, direction )
      }
  ...
  function dismiss ( callback, direction ) {
    ...
    currentStep().classList.remove( 'step-visited', 'active', 'enter' )
  ...
  registeredForms.push( function( event, trigger ) {
    ...
    if ( trigger === 'goto' ) {
      ...
      goTo( event.currentTarget.dataset.step - 1 )
      ...
      if ( form == formEl && !currentStep().querySelector( ':invalid' ) ) {
        ...
        if ( fireCallbacks ) {
          ...
          fireCallbacks( event, {
            ...
            go:       goTo,           // Go to a specific form index (1 based)
