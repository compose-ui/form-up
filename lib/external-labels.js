...
function change (event) {
  if ( event.currentTarget.outerHTML.match(/data-external-label/) )
...
function injectLabels ( input ) {
  ...
  for ( key in labels ) {
                                      // [data-input-label="disk"]
    var el = document.querySelector( '[data-input-label="'+key+'"]' )
...
function getLabels (el) {
  ...
  var labels = extractData( el, 'data-external-label' ),
      selected = {},
      labelNotFound
...
function watch(delay) {
  if ( !watching ) {
    Event.on( document, 'input', "input[step][data-has-external-labels]", Event.debounce(change, delay || 50) )
    Event.on( document, 'input', "input[type='range'][data-has-external-labels]", change )
    watching = true

    Event.ready(function(){
      var inputs = toolbox.slice(document.querySelectorAll('input[step],input[type="range"]'))
      inputs.forEach(function(input) {
        if ( extractData(input, 'data-external-label') != {}) {
          input.dataset.hasExternalLabels = 'true'
          injectLabels(input)
        }
      })
    })
