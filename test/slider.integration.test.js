...
describe('Slider', () => {
  beforeAll(async () => {
    await page.goto("http://localhost:8081/slider.html")
  ...
  it('receives an initial state', async () => {
    await expect(page).toMatchElement('#slider-1 input[type=range][data-input-state="initial"]')
  })
  ...
  it('only updates external labels when data-label="false"', async () => {
    ...
    await u.matchText(`#slider-8-container [data-step-label="disk"]`, '2GB')

    // Make sure external label is updated when changing slider value
    await u.setValue('#slider-8-container #slider-8 input', '2')
    await u.matchText(`#slider-8-container [data-step-label="disk"]`, '3GB')
  ...
  it('updates external labels with slider-state tracking', async () => {
    ...
    expect(await u.data(`#slider-8-container [data-track-input-state="#disk"]`, 'inputState')).toBe('increased')

    await u.setValue('#slider-8 input', '2')
    expect(await u.data(`#slider-8-container [data-track-input-state="#disk"]`, 'inputState')).toBe('decreased')

    await u.setValue('#slider-8 input', '3')
    expect(await u.data(`#slider-8-container [data-track-input-state="#disk"]`, 'inputState')).toBe('initial')
